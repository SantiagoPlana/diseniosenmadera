...
def generate_pdf(dic1, dic2):
    ...
    pdf.cell(20, 23, f'Contacto: {dic1["Contacto"]}', ln=True)
    pdf.set_font('helvetica', 'BU', 15)
    if len(dic1['Observaciones']) > 1:
        pdf.cell(43, 10, 'Observaciones:')
        pdf.set_font('helvetica', '', 15)
        pdf.multi_cell(200, 10, txt=f'{dic1["Observaciones"]}', ln=True)
    pdf.set_font('helvetica', 'BU', 15)
    pdf.cell(40, 20, 'Detalle:', ln=True)
    pdf.set_font('helvetica', '', 15)
    for k, v in dic2.items():
        pdf.cell(100, 10, f'{k} ')
        pdf.cell(100, 10, f'x{v[0]}')
        pdf.cell(70, 10, f'{v[1]}', ln=True)
    pdf.set_font('helvetica', 'B', 15)
    pdf.cell(200, 30, 'Total')
    pdf.cell(20, 30, f'{dic1["Total"]}')
    pdf.output(f'{dic1["Fecha"]}-{dic1["Cliente"]}.pdf')
